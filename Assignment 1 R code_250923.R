...
plot(rooted_tree, main = "Phylogenetic Tree of Bird Orders", cex = 0.6)
...
# The next code block uses the phylo.heatmap command from the phytools package to plot the phylogeny you've created against the site information you've just uploaded (this is possible because the tip labels of the phylogeny and the header of the column of the .csv file are the same values). First for the NJ tree. 

colors<-colorRampPalette(colors=c("white", "#1099dd"))(100)

phylo.heatmap(rooted_tree,genbank_seq_metadata, colors=colors)

# Don't worry if the tree and matrix overlap in the plot window - the sizing has been made for the final pdf output.  

# Now, use the same command structure to append the information to the ML tree. (Again, don't worry about overlap. 

phylo.heatmap(rooted_ml_tree,genbank_seq_metadata, colors=colors)
...
pdf("ZOO3700 deep-sea vent sequences with metadata - dispersal assignment - 250923.pdf", width = 18, height = 12) # Open a new pdf file
